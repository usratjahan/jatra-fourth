...
import travelHeader from "../../assets/images/CombinedHeader.png";
...
// TODO: GET /api/events/combined?division=&date=&travelers=&cities=&minPrice=&maxPrice=
// NOTE: This endpoint should return ONLY events where community = "Combined"
// Backend filter: WHERE community = 'Combined'
const MOCK_EVENTS = [
  {
    id: 1,
    ...
    community: "Combined", // Always "Combined" for this page
    division: "Chittagong",
  },
  {
    id: 2,
    ...
    description: "One of the few places where you can see both sunrise and sunset from the beach.",
    price: 8500,
    dateFrom: "15 Jul",
    dateTo: "18 Jul",
    time: "09:30",
  ...
  },
  {
    ...
    location: "Ratargul",
    sublocation: "Sylhet",
    description: "Explore Bangladesh's only freshwater swamp forest by boat — a hidden gem.",
    price: 5500,
    dateFrom: "10 Aug",
    dateTo: "12 Aug",
    time: "07:00",
  ...
  },
  {
    ...
    description: "Visit the only hilly island in Bangladesh with ancient temples and salt fields.",
    price: 7000,
    dateFrom: "5 Sep",
    dateTo: "7 Sep",
    time: "09:00",
...
const PriceRangeSlider = ({ min, max, value, onChange }) => {
  ...
  const handleTrackClick = (e) => {
    ...
    const distL   = Math.abs(clicked - value[0]);
    const distR   = Math.abs(clicked - value[1]);
    if (distL <= distR) onChange([Math.min(clicked, value[1] - 500), value[1]]);
    else                onChange([value[0], Math.max(clicked, value[0] + 500)]);
  };

  const startDrag = (side) => (e) => { e.preventDefault(); setDragging(side); };
  ...
  const onMouseMove = useCallback((e) => {
    ...
    if (dragging === "left")  onChange([Math.min(val, value[1] - 500), value[1]]);
    if (dragging === "right") onChange([value[0], Math.max(val, value[0] + 500)]);
    ...
    <div className="px-1">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-1.5 bg-red-500 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow">
          <span className="text-[10px] opacity-80">BDT</span>{fmt(value[0])}
        </div>
        <div className="flex items-center gap-1.5 bg-red-500 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow">
          <span className="text-[10px] opacity-80">BDT</span>{fmt(value[1])}
        </div>
      </div>
      <div ref={rangeRef} className="relative h-1.5 bg-gray-200 rounded-full cursor-pointer mx-2" onClick={handleTrackClick}>
        <div className="absolute h-full bg-red-500 rounded-full" style={{ left: `${leftPct}%`, right: `${100 - rightPct}%` }} />
        <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-4 h-4 bg-white border-2 border-red-500 rounded-full shadow cursor-grab active:cursor-grabbing hover:scale-110 transition-transform"
          style={{ left: `${leftPct}%` }} onMouseDown={startDrag("left")} onTouchStart={startDrag("left")} />
        <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-4 h-4 bg-white border-2 border-red-500 rounded-full shadow cursor-grab active:cursor-grabbing hover:scale-110 transition-transform"
          style={{ left: `${rightPct}%` }} onMouseDown={startDrag("right")} onTouchStart={startDrag("right")} />
      </div>
    ...
    <div className="bg-white rounded-[22px] overflow-hidden">
      ...
      <div className="relative m-3 rounded-2xl overflow-hidden h-52 bg-gradient-to-br from-teal-100 to-cyan-200">
        ...
        <div className="absolute top-3 right-3 flex items-center gap-1 bg-white rounded-xl px-2.5 py-1 shadow-md">
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24"
            fill="#ECC62D" stroke="#ECC62D" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round">
            <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
          </svg>
          <span className="text-gray-900 font-bold text-xs">{event.rating}</span>
        </div>
      </div>
      ...
      <div className="px-4 pb-4 pt-2">
        ...
        <div className="flex items-start gap-2 mb-3">
          ...
          <div>
            <p className="text-gray-900 font-bold text-lg leading-tight">{event.location}</p>
            {event.sublocation && (
              <p className="text-gray-700 text-base leading-tight">{event.sublocation}</p>
            )}
          </div>
        </div>

        {/* Date + Spots — Lucide CalendarDays + Users */}
        <div className="flex items-center gap-6 mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-700 text-sm font-medium">{event.dateFrom} - {event.dateTo}</span>
          </div>
          <div className="flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24"
              fill="none" stroke="#6B7280" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
            ...
            <span className="text-gray-700 text-sm font-medium">{event.spotsLeft} left</span>
          </div>
        </div>
...
// ═══════════════════════════════════════════════════════════
// SELECTED DIVISION PILLS (shown above cards)
// ═══════════════════════════════════════════════════════════
const DivisionSummaryCard = ({ divisions, onRemove }) => {
  ...
  return (
    <div className="mb-5 bg-white/90 backdrop-blur-sm rounded-2xl p-4 shadow-sm border border-teal-100">
      <p className="text-xs text-teal-600 font-semibold uppercase tracking-wider mb-2">Selected Divisions</p>
      <div className="flex flex-wrap gap-2">
        {divisions.map((d) => (
          <span key={d} className="flex items-center gap-1.5 px-3 py-1.5 bg-teal-600 text-white text-xs font-semibold rounded-full">
            📍 {d}
            <button onClick={() => onRemove(d)} className="hover:text-red-200 transition-colors font-bold ml-0.5">×</button>
          </span>
        ))}
      </div>
    </div>
...
const Combined = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // const res  = await fetch('/api/events/combined');
        // const data = await res.json();
        // NOTE: Backend should filter WHERE community = 'Combined'
        // setEvents(data); setFiltered(data);
        await new Promise((r) => setTimeout(r, 700));
        setEvents(MOCK_EVENTS);
        setFiltered(MOCK_EVENTS);
      } catch (err) {
        console.error("Fetch combined events failed:", err);
        setEvents(MOCK_EVENTS);
        setFiltered(MOCK_EVENTS);
      } finally {
        setLoading(false);
  ...
  const toggleDivision = (name) =>
    setSelectedDivisions((p) => p.includes(name) ? p.filter((d) => d !== name) : [...p, name]);
  ...
  const visibleCities = showAllCities ? filteredCities : filteredCities.slice(0, 6);
  ...
  // ── Reset ──
  const resetFilters = () => {
    setSelectedDivisions([]); setDate(""); setTravelers(2);
    setCitySearch(""); setSelectedCities([]);
  ...
  const inputCls = "w-full bg-white border border-gray-200 rounded-2xl px-4 py-3 text-sm text-gray-700 focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500 transition-colors";
  const labelCls = "block text-teal-700 font-semibold text-xs uppercase tracking-wider mb-1.5 px-1";
    ...
    <div className="relative bg-[#edfffd] min-h-screen pt-24 pb-16 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* <div className="mb-10 text-center">
          <p className="text-teal-600 font-semibold text-sm uppercase tracking-widest mb-2">Discover & Book</p>
          <h1 className="text-4xl sm:text-5xl font-bold text-gray-800 mb-3">Combined Community</h1>
          <p className="text-gray-500 text-base max-w-md mx-auto">Travel experiences open to everyone — all genders, all ages</p>
          Travel sticker image */}
          <div className="flex justify-center mt-5 mb-8">
            <img src={travelHeader} alt="Travel" className="w-full max-w-[1650px] h-auto object-contain drop-shadow-md" />
          </div>
        {/* </div> */}

        <div className="flex flex-col mt-4 lg:flex-row gap-6 items-start">

          {/* ════════════════════════════
              LEFT SIDEBAR
              NOTE: No community filter here — this page is always Combined
          ════════════════════════════ */}
              ...
              <div className="p-5 space-y-5">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {MOCK_DIVISIONS.map((div) => (
                      <button key={div.id} onClick={() => toggleDivision(div.name)}
                        ...
                        <div className={`w-5 h-5 rounded-md flex items-center justify-center border-2 flex-shrink-0 transition-all
                          ${selectedDivisions.includes(div.name) ? "bg-teal-600 border-teal-600" : "border-gray-300 group-hover:border-teal-400"}`}>
                        ...
                        <span className={`text-sm text-left transition-colors ${selectedDivisions.includes(div.name) ? "text-teal-700 font-medium" : "text-gray-600 group-hover:text-gray-900"}`}>
                          {div.name}
                        </span>
                      </button>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between bg-white border border-gray-200 rounded-2xl px-4 py-3">
                    <span className="text-gray-800 font-bold text-lg">{travelers}</span>
                    <div className="flex items-center gap-2">
                      <button onClick={() => setTravelers((p) => Math.max(1, p - 1))} disabled={travelers <= 1}
                      ...
                      <button onClick={() => setTravelers((p) => Math.min(20, p + 1))} disabled={travelers >= 20}
                        className="w-8 h-8 flex items-center justify-center rounded-full border-2 border-gray-200 text-gray-500 hover:border-teal-500 hover:text-teal-600 hover:bg-teal-50 disabled:opacity-30 transition-all">
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="border-t border-gray-100 pt-4">
                  ...
                  <div className="space-y-2">
                    {visibleCities.map((city) => (
                      <button key={city.id} onClick={() => toggleCity(city.name)}
                        ...
                        <span className={`text-sm text-left transition-colors ${selectedCities.includes(city.name) ? "text-teal-700 font-medium" : "text-gray-600 group-hover:text-gray-900"}`}>
                          {city.name}
                        </span>
                      </button>
                    ))}
                  ...
                  {filteredCities.length > 6 && (
                    <button onClick={() => setShowAllCities(!showAllCities)}
                      ...
                      {showAllCities ? "Show less" : `Show more (${filteredCities.length - 6} more)`}
                      <svg className={`w-4 h-4 transition-transform ${showAllCities ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="flex-1 min-w-0">
            ...
            <DivisionSummaryCard divisions={selectedDivisions} onRemove={removeDivision} />

            {/* Result bar */}
            <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
              <p className="text-gray-600 text-sm">
                Showing <span className="text-teal-700 font-bold">{filtered.length}</span> events
                {hasActiveFilters && " (filtered)"}
              </p>
              <div className="flex flex-wrap gap-2">
                {selectedCities.map((c) => (
                  <span key={c} className="flex items-center gap-1.5 px-3 py-1 bg-teal-100 text-teal-700 text-xs font-medium rounded-full">
                    🏙️ {c}
                    <button onClick={() => toggleCity(c)} className="font-bold hover:text-teal-900">×</button>
                  </span>
                ))}
                {(priceRange[0] > PRICE_MIN || priceRange[1] < PRICE_MAX) && (
                  <span className="flex items-center gap-1.5 px-3 py-1 bg-red-100 text-red-700 text-xs font-medium rounded-full">
                    💰 BDT {priceRange[0].toLocaleString()} – {priceRange[1].toLocaleString()}
                    <button onClick={() => setPriceRange([PRICE_MIN, PRICE_MAX])} className="font-bold hover:text-red-900">×</button>
                  </span>
                )}
              </div>
            </div>
